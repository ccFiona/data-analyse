...
input1=/data/test/bid-3.4.0-default/{$date_list}/*
echo $input1
output_path=/data/ott/tangye/clean_start
output=$output_path"/"$begin_date"_"$end_date


${HADOOP_HOME}/bin/hadoop fs -rmr $output
${HADOOP_HOME}/bin/hadoop jar ${HADOOP_HOME}/hadoop-streaming.jar \
  -D mapred.job.name="get_clean_start_tangye" \
  -D stream.num.map.output.key.fields=8 \
  -D num.key.fields.for.partition=1 \
  -input "$input1" \
  -output "$output" \
  -mapper "python getCleanStart_map.py" \
  -reducer "cat" \
  -file "getCleanStart_map.py" \
