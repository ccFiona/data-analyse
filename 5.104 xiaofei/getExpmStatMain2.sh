...
echo `date +%Y%m%d --date="-1 day"`

for((j=$interval;j>=1;j--))
do
...
#  cat $inputlogs_path/$tem/pv_$tem $inputlogs_path/$tem/start_$tem $inputlogs_path/$tem/error_$tem $inputlogs_path/$tem/vv_$tem > $inputlogs_path/active_tmp_$tem
#  i=$[$j+1]
